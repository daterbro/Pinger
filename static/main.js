...
function displaySearchResults(users) {
    ...
    if (limitedUsers.length > 0) {
        ...
        limitedUsers.forEach(user => {
            ...
            userSearchResults.appendChild(userDiv); // Добавляем div в блок результатов поиска
        });
    } else {
        // Если нет результатов, показываем кнопки чатов
...
// Поиск пользователя
function searchUser() {
    const query = userTagInput.value.toLowerCase();
...
userTagInput.oninput = function(event) {
    const userTag = userTagInput.value;
    event.preventDefault();
...
logoutButton.onclick = function() {
    ...
    localStorage.removeItem("userId"); // Удаление userId при выходе
...
function updateLanguage(lang) {
    const texts = {
        ru: {
            ...
            placeholder: "Введите уникальный юзернейм (начинается с @)",
            buttonLogout: "Выйти",
        },
        ua: {
            label: "Оберіть мову:",
            header: "Налаштування профілю",
            placeholder: "Введіть унікальний юзернейм (починається з @)",
            buttonLogout: "Вийти",
        },
        en: {
            label: "Select Language:",
            header: "Profile Settings",
            placeholder: "Enter a unique username (starts with @)",
            buttonLogout: "Logout",
        }
    };

    const selectedTexts = texts[lang];
    languageLabel.innerText = selectedTexts.label;
    settingsHeader.innerText = selectedTexts.header;
    usernameInput.placeholder = selectedTexts.placeholder;
    logoutButton.innerText = selectedTexts.buttonLogout;
}
...
// Инициализация WebSocket и получение userId из localStorage при загрузке страницы
window.onload = function() {
    userId = localStorage.getItem('userId'); // Инициализация userId из localStorage
    token = localStorage.getItem('token'); // Убедись, что 'token' правильно используется (регистр важен)
